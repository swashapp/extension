...
import {
  FilterStorage,
  PrivacyStorage,
} from '../../types/storage/privacy.type';
import { setBrowserIcon } from '../../utils/browser.util';
...
import { InitialFilters } from '../data/initial-filters';
...
export class PrivacyManager extends BaseStorageManager<PrivacyStorage> {
  ...
  private updateAppIcon(url?: string) {
    ...
    const paths = sizes.reduce(
      (acc, size) => {
        acc[size] = `static/images/swash/${status}-${size}.png`;
        return acc;
      },
