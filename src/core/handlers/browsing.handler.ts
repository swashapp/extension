...
import { BaseModuleHandler } from "../base/module.handler";
import { DataService } from "../services/data.service";

export class BrowsingHandler extends BaseModuleHandler<Handler.BROWSING> {
  ...
  constructor(
    ...
    super(Handler.BROWSING, onDiskModules, os);
  }
  ...
  private handleWebRequest(
    module: InMemoryModules<Handler.BROWSING>,
      ...
      try {
        switch (targetListener) {
          ...
          case "inspectVisit":
            ...
            break;
          default:
            console.warn(`Unknown target listener: ${targetListener}`);
        }

        if (res) await this.data.collect(res);
      } catch (error) {
        console.error(`Error in handleWebRequest callback: ${error}`);
      }
    };
        ...
        switch (p.type) {
          ...
          case "link":
            try {
              ...
              console.error(`Invalid URL: ${requestDetails.url}`);
            }
            break;
          case "referrer":
            ...
            break;
          default:
            console.warn(`Unknown parameter type: ${p.type}`);
        }
