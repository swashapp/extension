...
export class ContentHandler extends BaseModuleHandler<Handler.CONTENT> {
  constructor(onDiskModules: ModuleConfiguration, os: PlatformOS) {
    super(Handler.CONTENT, onDiskModules, os);
    this.scripts = ['/scripts/content.script.js'];
  ...
  public async getModules(url: string) {
    const modules: InMemoryModules<Handler.CONTENT>[] = [];
