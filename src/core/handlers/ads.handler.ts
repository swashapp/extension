...
export class AdsHandler extends BaseModuleHandler<Handler.ADS> {
  constructor(
    onDiskModules: ModuleConfiguration,
    os: PlatformOS,
    private preferences: PreferenceManager,
  ) {
    super(Handler.ADS, onDiskModules, os);
    this.scripts = ['/scripts/ads.script.js'];
  ...
  public async getModules(url: string) {
    const modules: InMemoryModules<Handler.ADS>[] = [];
