...
export class AjaxHandler extends BaseModuleHandler<Handler.AJAX> {
  ...
  constructor(
    ...
    super(Handler.AJAX, onDiskModules, os);
    this.scripts = ["/scripts/ajax.script.js"];
  ...
  private async handleWebRequest(
    module: InMemoryModules<Handler.AJAX>,
  ...
  public async getModules(url: string) {
    const modules: InMemoryModules<Handler.AJAX>[] = [];
