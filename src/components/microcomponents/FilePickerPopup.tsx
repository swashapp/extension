...
import 'url:../../statics/css/react-keyed-file-browser.css';
...
class FilePickerPopup extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      this.state.onboarding === 'GoogleDrive' ||
      this.state.onboarding === 'DropBox'
    ) {
  ...
  applyConfig() {
    ...
    if (this.state.selectedFile.id)
      return window.helper
        .downloadFile(this.state.onboarding, this.state.selectedFile.id)
        .then((response) => {
          if (response) {
            return window.helper
      ...
      switch (this.state.importState) {
        ...
        case 'waiting':
          return (
            <div
              disabled
              className="swash-transaction-modal-button swash-transaction-modal-button-waiting"
              onClick={this.applyConfig}
            >
              Importing...
            </div>
