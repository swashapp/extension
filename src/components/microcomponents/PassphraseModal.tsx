...
import loading from 'url:../../statics/images/loading.png';
...
class PassphraseModal extends React.Component {
  ...
  getBody() {
    switch (this.state.page) {
      case 1:
        return (
          <div
            className="swash-transaction-modal-body-large"
            style={{ overflow: 'auto', display: 'block', width: 'auto' }}
          >
            <div
              className="swash-modal-body-text"
              style={{ height: 'inherit' }}
            >
              This feature is experimental and so may not work perfectly all the
              time. It may change or be removed in the future.
              <p
                style={{
                  'font-weight': 'bold',
                }}
        ...
        return (
          ...
          >
            ...
            >
              ...
              seed phrase to access your 3Box backups. Save them somewhere safe
              and secret.
              <p
                style={{
                  display: 'block',
  ...
  signIn3Box() {
    const bip39 = require('bip39');
    this.setState({ waiting: true });
    const inputValue = document.getElementById('3box-passphrase-input').value;
  ...
  applyConfig() {
    if (this.state.selectedFile) {
      return window.helper
        .applyConfig(this.state.selectedFile.conf)
        .then((result) => {
          window.helper.save3BoxMnemonic(this.state.mnemonic).then(() => {
            ...
            this.notifyRef.current.handleNotification(
              'Can not import this config file',
              'error',
            );
