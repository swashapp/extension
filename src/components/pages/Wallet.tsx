...
class SettingsPage extends React.Component {
  ...
  transfer(e) {
    const amount = document.querySelector('#swash-amount').value;
    const recipient = document.querySelector('#swash-recipient').value;
    if (!amount.match(/^[0-9]+(\.[0-9]+)?$/)) {
      this.notifyRef.current.handleNotification(
      ...
      case 'RevealKey':
        if (this.isPrivateKeyRevealed()) {
          ...
          this.setState({
            ...
            revealFunction: { func: this.revealPrivateKey, text: 'reveal' },
          });
        break;
      case 'CopyKey':
        this.setState({
          ...
          revealFunction: {
            func: (e) => {
              const pKey = document.getElementById('swash-privateKey');
              const pKeyType = pKey.type;
              pKey.type = 'text';
              this.copyToClipboard(
                e,
                document.getElementById('swash-privateKey'),
              );
              pKey.type = pKeyType;
            },
            text: 'copy',
  ...
  async getBalanceInfo() {
    ...
    window.helper.getAvailableBalance().then((dataAvailable) => {
      dataAvailable =
        dataAvailable.error ||
        dataAvailable === '' ||
        typeof dataAvailable === 'undefined'
  ...
  revealPrivateKey(e) {
    const x = document.getElementById('swash-privateKey');
    if (x.type === 'password') {
      x.type = 'text';
    } else {
      x.type = 'password';
  ...
  isMessageNeeded() {
    return (
      this.state.dataAvailable !== '$' && Number(this.state.dataAvailable) > 0
    );
  }
  ...
  isTransferDisable() {
    if (
      this.state.dataAvailable === '$' ||
      Number(this.state.dataAvailable) <= 0
    )
      return true;
    if (!this.state.withdrawTo) return true;
    if (this.state.withdrawTo.value === 'Mainnet') {
      if (
        this.state.recipientEthBalance === '$' ||
        Number(this.state.recipientEthBalance) <= 0
      )
        return true;
      if (
  ...
  pasteWallet(e) {
    ...
    navigator.clipboard.readText().then(async (address) => {
      if (address.match(/^0x[a-fA-F0-9]{40}$/g)) {
        ...
        const DataBalance = await window.helper.getDataBalance(address);
        const EthBalance = await window.helper.getEthBalance(address);
        this.setState({
          recipient: address,
          recipientDataBalance: DataBalance,
  ...
  isClaimDisable() {
    return (
      this.state.unclaimedBonus === '$' ||
      Number(this.state.unclaimedBonus) <= 0 ||
      this.state.claimBtn === 'Claiming'
    );
  }
          ...
          <div className="swash-col">
            ...
            <div className="swash-setting-part">
              ...
              <div style={{ position: 'relative' }}>
                <input
                  type="text"
                  className="swash-form-input"
                  id="swash-wallet"
                  value={this.state.keyInfo.address}
                />
                <button
                  ...
                  onClick={(e) => {
                    this.copyToClipboard(
                      e,
                      document.getElementById('swash-wallet'),
                    );
                    e.target.focus();
              ...
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  className="swash-form-input"
                  id="swash-privateKey"
                  value={this.state.keyInfo.privateKey}
                />
                <RDropdownMenu
                  className="swash-button swash-form-input-button swash-reveal-button"
                  items={[
                    {
                      text: this.isPrivateKeyRevealed() ? 'Hide' : 'Reveal',
                      callback: () => this.openModal('RevealKey'),
                    },
                    {
                      text: 'Copy',
            ...
            <div className="swash-setting-part">
              ...
              <div className="swash-p">
                ...
                <br />
                <br />
                It’s important to make sure you have set up your wallet properly
                (it only takes a few minutes!). Check the{' '}
                <NavLink to="/Help">Help section</NavLink> for step-by-step
                instructions.
                <br />
                <br />
                ...
                <br />
                <br />
                Alternatively, if you use Ethereum, you will be presented with
                the amount needed in your wallet (in ETH) to cover the
                transaction fee. Exchange wallets are not currently supported.
                <br />
                <br />
              ...
              <div className="swash-transfer-row">
                ...
                <div className="swash-transfer-column swash-recipient-column">
                  ...
                  <div>
                    <input
                      ...
                      id="swash-recipient"
                      placeholder={
                        this.state.keyInfo.address.substr(0, 7) + '...'
                      }
                ...
                <div
                  className="swash-transfer-column swash-transfer-button-column"
                    ...
                    <ul>
                      {this.state.withdrawTo.value === 'xDai' ? (
                        <li className={'swash-text-red'}>
                          Make sure you have access to the recipient wallet.
                          Exchange wallets are not compatible with xDai.
                        </li>
                      ) : (
                        ''
                      )}
                      {Number(this.state.dataAvailable) >
                      this.state.minimumWithdraw ? (
                        <li className={'swash-text-green'}>
                          It’s on us. Swash will cover these transaction fees
                          for you! 🎉
                        </li>
                      ) : Number(this.state.recipientEthBalance) >
                        this.state.gasLimit ? (
                        <li className={'swash-text-orange'}>
                          Transaction fee is {this.state.gasLimit} ETH
                        </li>
                      ) : this.state.withdrawTo.value === 'Mainnet' ? (
                        <li className={'swash-text-orange'}>
                          Unable to withdraw - not enough ETH for the gas fee
                        </li>
                      ) : (
                        ''
                      )}
                      ...
                      this.state.recipient ? (
                        <li>
                          Balance:{' '}
                          {this.purgeNumber(this.state.recipientEthBalance)}{' '}
                          ETH,{' '}
                          {this.purgeNumber(this.state.recipientDataBalance)}{' '}
                          DATA
                        </li>
                      ) : (
                        ''
          ...
          {this.state.revealKeyModal ? (
            <div>
              ...
              >
                <RevealKeyModal
                  func={(e) => {
                    this.state.revealFunction.func(
                      e,
                      document.getElementById('swash-privateKey'),
                    );
                  }}
